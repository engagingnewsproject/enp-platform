"use strict";
(self["webpackChunkengage"] = self["webpackChunkengage"] || []).push([["assets_js_orbit_js"],{
...
var Orbit = /*#__PURE__*/function () {
  ...
  _createClass(Orbit, [{
    ...
    value: function init(ballsNum) {
      ...
      this.toggleAnimateBtn.addEventListener('click', this.toggleAnimate.bind(this)); // uncomment to have planet track cursor
    ...
    value: function step(timestamp) {
      ...
      for (var i = 0; i < this.balls.length; i++) {
        ...
        this.balls[i].style.transform = 'translate3d(' + xPos + 'px, ' + yPos + 'px, 0)'; // if these are true, then it's behind the planet
    ...
    value: function toggleAnimate() {
      ...
      if (this.animate) {
        this.toggleAnimateBtn.innerHTML = '<i class="fas fa-pause"></i>'; // resume the animation
    ...
    value: function setGravitationalPull(percent) {
      ...
      direction = percent < this.gravitationalPull ? '-' : '+'; // get the current pull and step it down over 20 steps so it's smoother than jumping straight there

      for (var i = 0; i < steps; i++) {
        // set the time this will fire
        time = i * (i / Math.PI); // minimum time span
        ...
        }, time); // on our last one, set the gravitationalPull to its final, nicely rounded number
