...
class Swas_Wp_Footnotes {
	...
	public function __construct() {
		...
		} elseif ( ! isset( $this->current_options['version'] ) || self::OPTIONS_VERSION !== $this->current_options['version'] ) {
		...
		$post_array = $_POST; // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function process( $data ) {
		...
		$display = true;
		if ( $this->current_options['no_display_home'] && is_home() ) {
			$display = false;
		}
		if ( $this->current_options['no_display_archive'] && is_archive() ) {
			$display = false;
		}
		if ( $this->current_options['no_display_date'] && is_date() ) {
			$display = false;
		}
		if ( $this->current_options['no_display_category'] && is_category() ) {
			$display = false;
		}
		if ( $this->current_options['no_display_search'] && is_search() ) {
			$display = false;
		}
		if ( $this->current_options['no_display_feed'] && is_feed() ) {
			$display = false;
		}
		if ( $this->current_options['no_display_preview'] && is_preview() ) {
			$display = false;
		}
		...
		foreach ( $identifiers as $key => $value ) {
			...
			$id_num     = ( 'decimal' === $style ) ? $value['use_footnote'] + $start_number : $this->convert_num( $value['use_footnote'] + $start_number, $style, count( $footnotes ) );
	 ...
	 * @since 1.0
	 */
	public function footnotes_options_page() {
	 ...
	 * @since 1.0
	 */
	public function footnotes_help() {
	 ...
	 */
	public function add_help_content() {
	 ...
	 */
	public function convert_num( $num, $style, $total ) {
	 ...
	 */
	public function roman( $num, $case = 'upper' ) {
		...
		return ( 'lower' === $case ) ? strtolower( $roman ) : $roman;
	}
	 ...
	 * @since 1.0.5
	 */
	public function alpha( $num, $case = 'upper' ) {
		$j = 1;
		for ( $i = 'A'; $i <= 'ZZ'; $i++ ) {
			if ( $j === $num ) {
				if ( 'lower' === $case ) {
