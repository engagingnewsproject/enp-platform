...
if (theme_coo) {
   ...
   createCookie('adce_theme','default',365);
   var theme = 'default';
} 
...
/// detect filetype
  var ace_file = document.getElementsByName( 'file' );
  var ace_file_type = 'application/x-httpd-php';

  if( ace_file && ace_file.length > 0 ) {
    if( ace_file[ 0 ].value.match( '\.css' ) ) {
      ace_file_type = 'text/css';
    }
    else if( ace_file[ 0 ].value.match( '\.js' ) ) {
      ace_file_type = 'text/javascript';
    }
  }
...
var editor = CodeMirror.fromTextArea(document.getElementById("newcontent"), {
	theme: theme,
	lineNumbers: true,
	matchBrackets: (ace_user.matchBrackets == 1) ? true : false,
	mode: ace_file_type,
	indentUnit: parseInt(ace_user.indentUnit),
	lineWrapping: (ace_user.lineWrapping == 1) ? true : false,
	tabSize: parseInt(ace_user.tabSize),
	indentWithTabs: true,
	enterMode: "keep",
	tabMode: "shift",
	onGutterClick: foldFunc,
	onChange: function(cm) { 
		if (jQuery('.ace_unsaved_changes').length){
			jQuery('.ace_file_status').removeClass('updated');
			jQuery('.ace_file_status').addClass('error');
		}else{
			jQuery('.ace_file_status').removeClass('updated');
			jQuery('.ace_file_status').addClass('error');
			if (jQuery('.ace_file_status').html() == ace_strings.noChangesyet)
				jQuery('.ace_file_status').html('<span class="ace_unsaved_changes"><small><strong>'+ace_strings.unsaved+'</strong></small></span>');
			else
				jQuery('.ace_file_status').append('<span class="ace_unsaved_changes"><small><strong>'+ace_strings.unsaved+'</strong></small></span>');
		}
	},
   extraKeys: {"Ctrl-Q": function(cm){foldFunc(cm, cm.getCursor().line);}},
   onKeyEvent: function(i, e) {
    // Hook into comment out
      if (e.keyCode == 191 && (e.ctrlKey || e.metaKey) && !e.altKey && e.shiftKey) {
    e.stop();
    return commentSelection(true);
      }
      // Hook into comment out
      if (e.keyCode == 190 && (e.ctrlKey || e.metaKey) && !e.altKey && e.shiftKey) {
    e.stop();
    return commentSelection(false);
      }
      ...
      // Hook into ctrl-m
      if (e.keyCode == 77 && (e.ctrlKey || e.metaKey) && !e.altKey) {
    e.stop();
    return autoFormatSelection();
      }
      // full screen.
	 ...
	 e.stop();
	 jQuery("#search").dialog({ title: 'Search Box' });
      }
      //replace
      if (e.keyCode == 72 && (e.ctrlKey || e.metaKey) && !e.altKey) {
	 e.stop();
	 jQuery("#searchR").dialog({ title: 'Search And Replace Box' });
	 return false;
      } 
      //save
	 ...
	 return false;
      ...
      //jump to line
      if (e.keyCode == 71 && (e.ctrlKey || e.metaKey) && !e.altKey) {
	 e.stop();
	 jQuery("#jump_tbox").dialog({ focus: function(event, ui){jQuery('#jump_line_number').focus(); }, hide: 'slide',title: 'Jump to Line', buttons: [
	{	text: "Jump",
   ...
   var ace_state = '';
   function mu(){ 
      var sta = editor.getTokenAt(editor.getCursor(false)); 
      var ace_current_state = sta['state']['curClose'].toString();
      if (ace_current_state == '/^<\/\s*script\s*>/i'){
	   ace_state= 'js';	
      }
      if (ace_current_state == '/^\?>/'){
	      ace_state= 'php';
      }
   }
   ...
   //alert(ace_state);
    ...
    if (!/^[\w$_]*$/.test(token.string)) {
      token = tprop = {start: cur.ch, end: cur.ch, string: "", state: token.state,
                       className: token.string == "." ? "cm-property" : null};
    }
    // If it is a property, find out what it is a property of.
    while (tprop.className == "cm-property") {
