(function ($) {
    ...
    var IRFeedbackApp = {
        ...
        getElements: function (slug) {
            ...
            $(".ir-feedback-dialog." + slug).find("input.reason-deactive:checked").each(function (index, ele) {
                reasonKey.push(this.name);
            });

            return {
                deactiveElement: $("#the-list").find('[data-slug="' + slug + '"] span.deactivate a'),
                modalElement: $(".ir-feedback-dialog." + slug),
                disableButton: $(".ir-feedback-dialog." + slug).find(".feedback-button a.disable-only"),
                sendFeedbackButton: $(".ir-feedback-dialog." + slug).find(".feedback-button a.send-message"),
                reasonKey: reasonKey,
                technicalElement: $(".ir-feedback-dialog." + slug).find(".technical-information")
            };
        },
            ...
            self.initModal = function (slug) {
                ...
                if (!modal) {
                    modal = self.getElements(slug).modalElement.dialog({
                        ...
                        modal: true,
                        dialogClass: 'noTitle irDialogFeedback',
                        show: {
                            effect: "fade",
                            duration: 500
                        },
                        ...
                        create: function (event, ui) {
                            ...
                            self.getElements(slug).disableButton.click(function (e) {
                                ...
                                $.ajax({
                                    ...
                                    method: 'POST',
                                    data: {
                                        action: 'ir_disable_feedback_' + slug,
                                        ajax_nonce: ir_feedback.token
                                    },
                                    success: function () {
                            ...
                            self.getElements(slug).sendFeedbackButton.click(function (e) {
                                ...
                                e.preventDefault();
                                self.getElements(slug).modalElement.find('.ir-loading').show();
                                $.ajax({
                                    url: ajaxurl,
                                    dataType: 'json',
                                    method: 'POST',
                                    data: {
                                        action: 'ir_send_feedback_deactive_' + slug,
        ...
        renderTechnicalData: function renderTechnicalData(slug) {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {
                    action: 'ir_feedback_get_technical_data_' + slug,
                    ajax_nonce: ir_feedback.token
                },
                success: function (res) {
                    if (res.get_status) {
