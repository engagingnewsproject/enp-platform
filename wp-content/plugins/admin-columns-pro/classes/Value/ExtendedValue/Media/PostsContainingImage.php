...
class PostsContainingImage implements ExtendedValue
{

    private const NAME = 'posts-containing-image';

    private PostsContainingImageFinder $finder;

    private PostsModalRenderer $renderer;

    public function __construct(PostsContainingImageFinder $finder, PostsModalRenderer $renderer)
    {
        $this->finder = $finder;
    ...
    public function render($id, array $params, Column $column, ListScreen $list_screen): string
    {
        ...
        $title = sprintf(
            _n('%d post contains this image', '%d posts contain this image', $total, 'codepress-admin-columns'),
            $total
        );
