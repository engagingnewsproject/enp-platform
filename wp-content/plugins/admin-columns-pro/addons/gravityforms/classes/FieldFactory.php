...
final class FieldFactory
{
    ...
    public function create_by_column_type(string $column_type, int $form_id): ?Field\Field
    {
        $field_id = str_replace('field_id-', '', $column_type);

        return $this->create($field_id, $form_id);
    }

    public function create(string $field_id, int $form_id): ?Field\Field
    {
    ...
    private function get_field_for_gf_field(GF_Field $gf_field, int $form_id, string $field_id): ?Field\Field
    {
        switch ($gf_field->offsetGet('type')) {
            ...
            case FieldTypes::QUANTITY:
                return $gf_field->get_input_type() === 'select'
                    ...
                    : new Field\Type\Number($form_id, $field_id, $gf_field);
    ...
    private function is_sub_field(string $field_id): bool
    {
