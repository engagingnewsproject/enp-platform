 ...
 */
class CME_Recent_Posts extends WP_Widget
{
    public function __construct()
    {
        $widget_ops = array('classname' => 'cme_recent_posts', 'description' => __("Latest CPT's & Posts.") );
        parent::__construct('cme-recent-posts', __('CME Recent Posts'), $widget_ops);
        $this->alt_option_name = 'cme_recent_posts';
    ...
    public function widget($args, $instance)
    {
        $cache = array();
        if (! $this->is_preview()) {
            $cache = wp_cache_get('cme_widget_recent_posts', 'widget');
        }
...
<?php
        ...
        if (! $this->is_preview()) {
            ...
            wp_cache_set('cme_widget_recent_posts', $cache, 'widget');
        } else {
            ob_end_flush();
        }
    }
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        if (isset($alloptions['cme_recent_posts'])) {
            delete_option('cme_recent_posts');
        }
    ...
    {
        wp_cache_delete('cme_widget_recent_posts', 'widget');
    }
...
function cme_recent_posts_widget_init()
{
    if (!is_blog_installed()) {
        return;
    }

    register_widget('CME_Recent_Posts');
    do_action('widgets_init');
}

add_action('init', 'cme_recent_posts_widget_init', 2);

/**
 * Include CSS file for MyPlugin.
 */
function cme_recent_posts_enqueue_styles()
{
    wp_register_style('style', plugin_dir_url(__FILE__) . 'assets/style.css');
    wp_enqueue_style('style');
}
add_action('wp_enqueue_scripts', 'cme_recent_posts_enqueue_styles');
