...
EnpIframeQuiz.prototype.sendParentURL = function() {
    ...
    if(/https?:\/\/(?:local.quiz|(?:(?:local|dev|test)\.)?(engagingnewsproject|mediaengagement)\.org|(?:engagingnews|enpdev|cmengage)\.(?:staging\.)?wpengine\.com)\/enp-quiz\/quiz-preview\/\d+\b/.test(this.parentURL)) {
...
function handleEnpIframeMessage(event) {
    ...
    if(!/https?:\/\/(?:local.quiz|(?:(?:local|dev|test)\.)?(engagingnewsproject|mediaengagement)\.org|(?:(?:local|dev|test)\.)?mediaengagement\.org|(?:engagingnews|enpdev|cmengage)\.(?:staging\.)?wpengine\.com)\b/.test(event.origin)) {
        return false;
    }
