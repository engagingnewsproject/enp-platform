...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('NF_FU_VENDOR\\Firebase\\JWT\\JWT')) {
            return \NF_FU_VENDOR\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \NF_FU_VENDOR\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm)
    {
        if (\class_exists('NF_FU_VENDOR\\Firebase\\JWT\\JWT')) {
            return \NF_FU_VENDOR\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm);
        }
        return \NF_FU_VENDOR\JWT::encode($assertion, $signingKey, $signingAlgorithm);
    }
    /**
