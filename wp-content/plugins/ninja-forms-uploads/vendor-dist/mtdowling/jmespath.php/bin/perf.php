...
$xdebug = new \NF_FU_VENDOR\Composer\XdebugHandler\XdebugHandler('perf.php');
$xdebug->check();
unset($xdebug);
$dir = isset($argv[1]) ? $argv[1] : __DIR__ . '/../tests/compliance/perf';
\is_dir($dir) or die('Dir not found: ' . $dir);
// Warm up the runner
\NF_FU_VENDOR\JmesPath\Env::search('foo', []);
$total = 0;
...
function runCase($given, $expression, $name)
{
    ...
    for ($i = 0; $i < 100; $i++) {
        ...
        if (!\getenv('CACHE')) {
            ...
            if ($runtime instanceof \NF_FU_VENDOR\JmesPath\CompilerRuntime) {
