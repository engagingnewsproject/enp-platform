 ...
 */
namespace NF_FU_LIB\Google\AccessToken;

use NF_FU_LIB\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use NF_FU_LIB\Firebase\JWT\SignatureInvalidException;
use NF_FU_LIB\GuzzleHttp\Client;
use NF_FU_LIB\GuzzleHttp\ClientInterface;
use NF_FU_LIB\phpseclib3\Crypt\PublicKeyLoader;
use NF_FU_LIB\phpseclib3\Crypt\RSA\PublicKey;
use NF_FU_LIB\Psr\Cache\CacheItemPoolInterface;
use NF_FU_LIB\Google\Auth\Cache\MemoryCacheItemPool;
use NF_FU_LIB\Google\Exception as GoogleException;
use NF_FU_LIB\Stash\Driver\FileSystem;
use NF_FU_LIB\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use NF_FU_LIB\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('NF_FU_LIB\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'NF_FU_LIB\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('NF_FU_LIB\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('NF_FU_LIB\\phpseclib3\\Crypt\\RSA')) {
            return 'NF_FU_LIB\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('NF_FU_LIB\\phpseclib\\Crypt\\RSA')) {
            return 'NF_FU_LIB\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('NF_FU_LIB\\phpseclib3\\Math\\BigInteger')) {
            return 'NF_FU_LIB\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('NF_FU_LIB\\phpseclib\\Math\\BigInteger')) {
            return 'NF_FU_LIB\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('NF_FU_LIB\\phpseclib3\\Crypt\\AES')) {
            return 'NF_FU_LIB\\phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('NF_FU_LIB\\phpseclib\\Crypt\\RSA')) {
            return 'NF_FU_LIB\\phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('NF_FU_LIB\\Crypt_RSA')) {
