...
use NF_FU_LIB\Aws\ResponseContainerInterface;
...
abstract class AbstractMonitoringMiddleware implements MonitoringMiddlewareInterface
{
    ...
    public function __invoke(CommandInterface $cmd, RequestInterface $request)
    {
        ...
        return Create::promiseFor($handler($cmd, $request))->then($g, $g);
    }
    ...
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) \floor(\microtime(\true) * 1000), 'UserAgent' => \substr($request->getHeaderLine('User-Agent') . ' ' . \NF_FU_LIB\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
