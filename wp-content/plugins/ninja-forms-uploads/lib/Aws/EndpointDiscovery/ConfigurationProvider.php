...
namespace NF_FU_LIB\Aws\EndpointDiscovery;

use NF_FU_LIB\Aws\CacheInterface;
use NF_FU_LIB\Aws\EndpointDiscovery\Exception\ConfigurationException;
...
class ConfigurationProvider
{
    ...
    public static function env($cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        return function () use($cacheLimit) {
            ...
            if ($enabled !== \false && $enabled !== '') {
                return Create::promiseFor(new Configuration($enabled, $cacheLimit));
            }
    ...
    {
        return function () {
            return Create::promiseFor(new Configuration(self::DEFAULT_ENABLED, self::DEFAULT_CACHE_LIMIT));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null, $cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        ...
        return function () use($profile, $filename, $cacheLimit) {
            ...
            return Create::promiseFor(new Configuration($data[$profile]['endpoint_discovery_enabled'], $cacheLimit));
        };
    }
    /**
