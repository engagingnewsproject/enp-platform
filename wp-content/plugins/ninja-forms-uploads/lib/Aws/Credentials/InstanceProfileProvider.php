...
use NF_FU_LIB\Aws\Exception\InvalidJsonException;
use NF_FU_LIB\Aws\Sdk;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->attempts = 0;
        $this->client = isset($config['client']) ? $config['client'] : \NF_FU_LIB\Aws\default_http_handler();
    }
    /**
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        $userAgent .= ' ' . \NF_FU_LIB\Aws\default_user_agent();
        ...
        return $fn($request, ['timeout' => $this->timeout])->then(function (ResponseInterface $response) {
            ...
            if ($reason instanceof \NF_FU_LIB\GuzzleHttp\Exception\RequestException) {
                throw $reason;
            }
