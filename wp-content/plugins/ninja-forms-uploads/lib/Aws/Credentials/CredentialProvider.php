...
namespace NF_FU_LIB\Aws\Credentials;

use NF_FU_LIB\Aws;
use NF_FU_LIB\Aws\Api\DateTimeResult;
use NF_FU_LIB\Aws\CacheInterface;
use NF_FU_LIB\Aws\Exception\CredentialsException;
use NF_FU_LIB\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        return self::memoize(\call_user_func_array(self::class.'::chain', $defaultChain));
    }
    /**
    ...
    public static function cache(callable $provider, CacheInterface $cache, $cacheKey = null)
    {
        ...
        return function () use($provider, $cache, $cacheKey) {
            ...
            if ($found instanceof CredentialsInterface && !$found->isExpired()) {
                return Create::promiseFor($found);
            }
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return Create::promiseFor(new Credentials($key, $secret, \getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return Create::promiseFor(new Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \NF_FU_LIB\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
            if ($data === \false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            return Create::promiseFor(new Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    {
        $profileData = \NF_FU_LIB\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && \getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \NF_FU_LIB\Aws\parse_ini_file($configFilename, \true, \INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (\file_exists($credFile)) {
            $profiles = \NF_FU_LIB\Aws\parse_ini_file($credFile, \true, \INI_SCANNER_RAW);
        }
        if (\file_exists($configFile)) {
            $configProfileData = \NF_FU_LIB\Aws\parse_ini_file($configFile, \true, \INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = \str_replace('profile ', '', $name);
