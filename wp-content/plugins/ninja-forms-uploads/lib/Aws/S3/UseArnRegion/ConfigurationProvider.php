...
namespace NF_FU_LIB\Aws\S3\UseArnRegion;
...
use NF_FU_LIB\Aws\S3\UseArnRegion\Exception\ConfigurationException;
...
class ConfigurationProvider extends AbstractConfigurationProvider implements ConfigurationProviderInterface
{
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($useArnRegion)) {
                return Create::promiseFor(new Configuration($useArnRegion));
            }
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \NF_FU_LIB\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
            if ($data === \false) {
            ...
            return Create::promiseFor(new Configuration($data[$profile][self::INI_USE_ARN_REGION]));
        };
    }
    public static function fallback()
    {
        return function () {
            return Create::promiseFor(new Configuration(self::DEFAULT_USE_ARN_REGION));
        };
    }
}
