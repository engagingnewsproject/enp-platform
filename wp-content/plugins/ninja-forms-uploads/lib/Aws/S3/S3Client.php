...
use NF_FU_LIB\Aws\Api\DocModel;
use NF_FU_LIB\Aws\Api\Service;
use NF_FU_LIB\Aws\AwsClient;
use NF_FU_LIB\Aws\CacheInterface;
use NF_FU_LIB\Aws\ClientResolver;
use NF_FU_LIB\Aws\Command;
use NF_FU_LIB\Aws\Exception\AwsException;
use NF_FU_LIB\Aws\HandlerList;
use NF_FU_LIB\Aws\Middleware;
use NF_FU_LIB\Aws\RetryMiddleware;
use NF_FU_LIB\Aws\ResultInterface;
use NF_FU_LIB\Aws\CommandInterface;
use NF_FU_LIB\Aws\S3\UseArnRegion\Configuration;
use NF_FU_LIB\Aws\S3\UseArnRegion\ConfigurationInterface;
use NF_FU_LIB\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use NF_FU_LIB\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use NF_FU_LIB\GuzzleHttp\Exception\RequestException;
use NF_FU_LIB\GuzzleHttp\Promise\Promise;
...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    public function createPresignedRequest(CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\NF_FU_LIB\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \NF_FU_LIB\Aws\serialize($command)->getUri();
    }
    /**
