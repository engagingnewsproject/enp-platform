...
use NF_FU_LIB\Aws\LruArrayCache;
use NF_FU_LIB\Aws\Result;
use NF_FU_LIB\Aws\S3\Exception\S3Exception;
use NF_FU_LIB\GuzzleHttp\Psr7;
use NF_FU_LIB\GuzzleHttp\Psr7\MimeType;
use NF_FU_LIB\GuzzleHttp\Psr7\Stream;
use NF_FU_LIB\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = MimeType::fromExtension($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \NF_FU_LIB\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (Result $result) use($filterFn) {
