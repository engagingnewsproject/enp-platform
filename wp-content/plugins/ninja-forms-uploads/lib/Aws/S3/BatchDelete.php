...
use NF_FU_LIB\Aws\AwsClientInterface;
use NF_FU_LIB\Aws\S3\Exception\DeleteMultipleObjectsException;
use NF_FU_LIB\GuzzleHttp\Promise;
use NF_FU_LIB\GuzzleHttp\Promise\Create;
use NF_FU_LIB\GuzzleHttp\Promise\Coroutine;
use NF_FU_LIB\GuzzleHttp\Promise\PromisorInterface;
...
class BatchDelete implements PromisorInterface
{
    ...
    {
        $fn = function (BatchDelete $that) use($iter) {
            return Coroutine::of(function () use($that, $iter) {
                foreach ($iter as $obj) {
    ...
    private function createPromise()
    {
        ...
        return $promise->then(function () use($cleanup) {
            return Create::promiseFor($this->flushQueue())->then($cleanup);
