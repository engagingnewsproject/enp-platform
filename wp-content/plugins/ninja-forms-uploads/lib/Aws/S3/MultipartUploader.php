...
use NF_FU_LIB\Aws\Multipart\AbstractUploader;
...
use NF_FU_LIB\Psr\Http\Message\StreamInterface as Stream;
use NF_FU_LIB\Aws\S3\Exception\S3MultipartUploadException;
/**
...
class MultipartUploader extends AbstractUploader
{
    ...
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return MimeType::fromFilename($uri) ?: 'application/octet-stream';
        }
    }
