...
use NF_FU_LIB\Aws\S3\MultipartUploader;
use NF_FU_LIB\Aws\S3\S3ClientInterface;
...
class S3EncryptionMultipartUploader extends MultipartUploader
{
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = Create::promiseFor($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
